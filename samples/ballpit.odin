...
@(private)
build_wall :: proc(body_interface: ^jph.BodyInterface, size: [3]f32, position: [3]f32) -> jph.BodyID {
    size := size
    position := position
    ...
    return jph.BodyInterface_CreateAndAddBody(body_interface, floor_settings, .JPH_Activation_DontActivate)
}
...
main :: proc() {
    ...
    when ODIN_DEBUG {
        ...
        defer {
            ...
            for bad_free in track_alloc.bad_free_array {
                fmt.printfln("%v allocation %p was freed badly\n", bad_free.location, bad_free.memory)
            }
        }
    }
    ...
    jph.ObjectLayerPairFilterTable_EnableCollision(object_layer_pair_filter, OBJECT_LAYER_MOVING, OBJECT_LAYER_MOVING)
    ...
    floor_id: jph.BodyID
    {
        ...
        floor_id = jph.BodyInterface_CreateAndAddBody(body_interface, floor_settings, .JPH_Activation_DontActivate)
    }
    ...
    for !rl.WindowShouldClose() {
        ...
        if rl.IsMouseButtonPressed(.LEFT) {
            ...
            if (jph.NarrowPhaseQuery_CastRay(narrow_phase_query, &ray.position, &direction, &result, nil, nil, nil)) {
        ...
        if is_spawning {
            ball_pos := [3]f32{rand.float32_range(-22, 22), rand.float32_range(20, 30), rand.float32_range(-22, 22)}
            ...
            ball_id := jph.BodyInterface_CreateAndAddBody(body_interface, sphere_settings, .JPH_Activation_Activate)
