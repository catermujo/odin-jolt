...
hello_world :: proc(t: ^testing.T) {
    ...
    ObjectLayerPairFilterTable_EnableCollision(object_layer_pair_filter, OBJECT_LAYER_MOVING, OBJECT_LAYER_MOVING)
    ObjectLayerPairFilterTable_EnableCollision(object_layer_pair_filter, OBJECT_LAYER_MOVING, OBJECT_LAYER_NON_MOVING)

    broad_phase_layer_interface_table := BroadPhaseLayerInterfaceTable_Create(OBJECT_LAYER_NUM, BROAD_PHASE_LAYER_NUM)
    ...
    proc "c" (userData: rawptr, body1: ^Body, body2: ^Body, manifold: ^ContactManifold, settings: ^ContactSettings) {
        context = runtime.default_context()
    ...
    my_contact_listener_procs.OnContactRemoved = proc "c" (userData: rawptr, subShapePair: ^SubShapeIDPair) {
        context = runtime.default_context()
    ...
    my_activation_listener_proc.OnBodyActivated = proc "c" (userData: rawptr, bodyID: BodyID, bodyUserData: u64) {
        context = runtime.default_context()
        fmt.println("[BodyActivationListener] A body got activated")
    }
    my_activation_listener_proc.OnBodyDeactivated = proc "c" (userData: rawptr, bodyID: BodyID, bodyUserData: u64) {
        context = runtime.default_context()
    ...
    floor_id: BodyID
    {
        ...
        floor_id = BodyInterface_CreateAndAddBody(body_interface, floor_settings, .JPH_Activation_DontActivate)
    }
    ...
    sphere_id: BodyID
    {
        ...
        sphere_id = BodyInterface_CreateAndAddBody(body_interface, sphere_settings, .JPH_Activation_Activate)
    }
